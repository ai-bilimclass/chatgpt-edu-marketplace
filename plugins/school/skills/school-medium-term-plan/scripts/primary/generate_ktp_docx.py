...
OBJECTIVE_CODE = re.compile(r"(?<!\d)[1-4](?:\.\d+){3}(?!\d)")
...
SUBJECT_PROFILE_PATTERNS = {
    "languages_literature": re.compile(
        r"тілі|әдебиет|сауат ашу|ана тілі|әліппе|язык|литератур|обучение грамоте|букварь|родная речь|"
        r"language|literature|literacy|english|german|french|ағылшын|неміс|француз|ұйғыр|өзбек|тәжік",
        re.IGNORECASE,
    ),
    "mathematics_informatics": re.compile(
        r"математ|цифрлық сауат|цифровая грамот|digital literacy|алгебр|геометр|информат|"
        r"mathemat|algebra|geometry|informatics|computer science",
        re.IGNORECASE,
    ),
    "natural_sciences": re.compile(
        r"жаратылыстану|дүниетану|естествозн|познание мира|физик|хими|биологи|географ|natural science|world studies|"
    ...
    "arts_technology": re.compile(
        r"музык|көркем еңбек|еңбекке баулу|художественн|трудовое обучение|графика|жобалау|черчение|технолог|"
...
def validate_subject_context(data: dict[str, Any]) -> None:
    ...
    if "500" not in str(data["curriculum_plan_order"]):
        raise ValueError("curriculum_plan_order must identify the applicable order №500")
    if not str(data["curriculum_plan_variant"]).strip():
        raise ValueError("curriculum_plan_variant must identify the applicable №500 plan variant")
    try:
        date.fromisoformat(str(data["curriculum_plan_revision_date"]))
    except ValueError as exc:
        raise ValueError("curriculum_plan_revision_date must use YYYY-MM-DD") from exc
    ...
    if direction != "not_applicable":
        raise ValueError("profile_direction must be not_applicable for primary grades")
...
def assessment_language(data: dict[str, Any]) -> str:
    document_language = str(data["language"])
    selected = str(data.get("assessment_language", document_language)).strip()
    if document_language == "trilingual":
        if selected not in {"kk", "ru", "en"}:
            raise ValueError(
                "Үштілді КТП үшін assessment_language мәнін kk, ru немесе en деп таңдаңыз"
            )
        return selected
    if selected != document_language:
        raise ValueError("assessment_language must match a monolingual document language")
...
def validate(data: dict[str, Any]) -> None:
    ...
    required = [
        ...
        "curriculum_order", "curriculum_appendix", "curriculum_revision_date",
        "curriculum_plan_order", "curriculum_plan_variant", "curriculum_plan_revision_date",
        "subject_profile",
        ...
        "cross_cutting_themes", "lesson_topics_confirmed_by_teacher",
    ...
    if int(data["grade"]) not in range(1, 5):
        raise ValueError("Grade must be from 1 to 4")
    ...
    literacy_subject = int(data["grade"]) == 1 and str(data["subject"]).strip().casefold() in {
        "әліппе", "букварь"
    }
    if source_type not in {
        "teacher_uploaded_program", "teacher_provided_objectives",
        "teacher_provided_sections_and_topics",
    }:
        raise ValueError(
            "Primary KTP requires a teacher-uploaded program or a complete set of "
            "teacher-provided objectives for all quarters"
        )
    if not isinstance(data["official_program_verified"], bool):
        raise ValueError("official_program_verified must be boolean")
    if source_type == "teacher_uploaded_program":
        if data["official_program_verified"] is not True:
            raise ValueError("The uploaded official program must be verified")
    ...
    if source_type == "teacher_provided_sections_and_topics":
        if not literacy_subject:
            raise ValueError("teacher_provided_sections_and_topics is allowed only for grade 1 Әліппе/Букварь")
        if not str(data["teacher_program_file"]).strip():
            raise ValueError("Әліппе/Букварь requires the teacher's uploaded sections, lesson topics, and objectives")
    ...
    themes = data["cross_cutting_themes"]
    organizer_type = data.get("content_organizer_type", "cross_cutting_themes")
    organizers = data.get("content_organizers", themes)
    if literacy_subject:
        if organizer_type != "sections":
            raise ValueError("Grade 1 Әліппе/Букварь must use teacher-provided sections, not cross-cutting themes")
        if themes not in ([], None):
            raise ValueError("Grade 1 Әліппе/Букварь must not use cross-cutting themes")
    elif organizer_type != "cross_cutting_themes":
        raise ValueError("Other primary subjects must use cross-cutting themes")
    if not isinstance(organizers, list) or not organizers or any(not str(item).strip() for item in organizers):
        raise ValueError("Primary KTP requires a non-empty list of content organizers")
    normalized_themes = {str(item).strip().casefold() for item in organizers}
    if data["lesson_topics_confirmed_by_teacher"] is not True:
        raise ValueError(
            "Primary KTP lesson topics must be explicitly confirmed or corrected by the teacher"
        )
    for field in ("subject", "content_source"):
    ...
    expected_weeks = [8, 8, 9, 8] if int(data["grade"]) == 1 else EXPECTED_WEEKS
    ...
    if not isinstance(data["summative_assessment_required"], bool):
        raise ValueError("summative_assessment_required must be boolean")
    # Primary KTP does not display secondary-school summative labels. Keep the
    # legacy input fields for compatibility, but normalize them unconditionally.
    ...
    for quarter in quarters:
        ...
        for lesson in quarter["lessons"]:
            lesson["assessment_type"] = ""
    for lesson in lessons:
        lesson_theme = str(lesson.get("section", "")).strip()
        if not lesson_theme:
            raise ValueError("Every lesson must have a section or cross-cutting theme")
        if lesson_theme.casefold() not in normalized_themes:
            raise ValueError(
                f"Lesson theme {lesson_theme!r} is not listed in content organizers"
            )
    ...
    if abs(expected - expected_year) > 1e-9:
        raise ValueError(
            f"Total hours ({expected:g}) must equal {sum(expected_weeks)} weeks × hours_per_week ({expected_year:g})"
        )
...
def assign_lesson_dates(data: dict[str, Any]) -> None:
    ...
    for quarter in data["quarters"]:
        ...
        if len(assignments) < needed:
            ...
            # begin after one of this subject's weekly slots (for example,
            # 01 September is Tuesday while the subject also meets Monday).
            # Preserve the official hour total by combining the missing opening
            # slot with the first actual lesson date and make that visible.
            if not assignments:
