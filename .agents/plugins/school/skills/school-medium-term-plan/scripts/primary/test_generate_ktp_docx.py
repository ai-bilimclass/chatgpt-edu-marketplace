...
class DynamicHoursValidationTest(unittest.TestCase):
    def test_rejects_external_url_anywhere_in_ktp_input(self):
        data = make_data(1)
        data["content_source"] = "https://example.org/program.pdf"
    ...
    def test_rejects_external_url_field_in_nested_calendar_data(self):
        data = make_data(1)
        data["source_conflicts"] = [{"url": "teacher-file-reference"}]
    ...
    def test_official_calendar_reference_contains_order_and_holidays(self):
        ...
        holidays = json.loads((root / "references" / "calendar" / "official-holidays-2026-2027.json").read_text(encoding="utf-8"))
